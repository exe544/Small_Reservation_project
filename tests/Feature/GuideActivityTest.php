...
class GuideActivityTest extends TestCase
{
    ...
    public function test_guides_can_view_activities_only_assigned_to_him()
    {
        $company = Company::factory()->create();
        $user = User::factory()->guide()->create(['company_id' => $company->id,]);
        $activity = Activity::factory()->create(['guide_id' => $user->id, 'company_id' => $company->id]);
        $activity2 = Activity::factory()->withCompanyAndGuide()->create();
    ...
    public function test_guide_sees_activities_ordered_by_time_correctly()
    {
        ...
        $activity = Activity::factory()->create(['guide_id' => $user->id, 'company_id' => $company->id, 'start_date' => now()->addWeek()]);
        $activity2 = Activity::factory()->create(['guide_id' => $user->id, 'company_id' => $company->id, 'start_date' => now()->addMonth()]);
        $activity3 = Activity::factory()->create(['guide_id' => $user->id, 'company_id' => $company->id, 'start_date' => now()->addMonths(2)]);
